...
**Generated HTML.** The HTML generated for a UI element will be wrapped in a
special HTML tag, `<UIElement>`. This element will carry an attribute called
`object-id` which contains the Python object `id` of the corresponding
`UIElement` object.

**Frontend synchronization.** XXX whatever we do in the frontend right now
should continue to work, but ... check this XXX
